...
- (void)viewDidLoad {
    ...
    self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"车库全部" style:UIBarButtonItemStylePlain target:self action:@selector(watchCars)];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (person.number == 0) {
        ...
        cell.textLabel.text = [NSString stringWithFormat:@"%@(第%ld次更新)",person.name,(long)person.number];
    }
    
    
    cell.detailTextLabel.text = [NSString stringWithFormat:@"age: %ld",(long)person.age];
